"""
═══════════════════════════════════════════════════════════════════════════════
  SMA OUTFIT DETECTION ENGINE — NORMALIZED
  Element 47 — single-file, Webull OpenAPI

  Timeframe-equal scoring variant. Identical to engine.py in every respect
  except the ranking formula: decisecond scores are divided by the timeframe's
  own decisecond baseline so that a 1mo hit and a 1m hit contribute equally
  (1.0 each) before freshness weighting. No timeframe is inflated by candle
  duration. Use this engine when you want 1m signals to compete on equal footing
  with 1w/1mo signals.

  Original engine (engine.py) is unchanged. This file is a separate standalone.

  Commands:
    docker exec e47_engine python /app/engine_normalized.py
    docker exec e47_engine python /app/engine_normalized.py --source webull --universe all
    docker exec e47_engine python /app/engine_normalized.py --top-n 25 --json

  Output: output/normalized_engine/
═══════════════════════════════════════════════════════════════════════════════
...
OUTFITS: list[dict] = [
    ...
    {"id": 43, "periods": [21, 42, 83, 116, 332, 664],           "name": "SPYU 4X"},
]
...
class WebullClient(DataClient):
    ...
    def _init_sdk(self) -> None:
        """Initialize Webull SDK with a 30-second timeout.
        Falls back to manual REST if SDK hangs or is unavailable."""
        ...
        def _try_sdk():
            ...
            sdk_client = SDKDataClient(api_client)
            return sdk_client

        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(_try_sdk)
                sdk_client = future.result(timeout=30)
            self._sdk_data_client = sdk_client
            self._sdk_available = True
            logging.info("Webull SDK initialised successfully")
        except concurrent.futures.TimeoutError:
            logging.warning("Webull SDK init timed out after 30s — using REST fallback")
            self._sdk_available = False
        except ImportError:
            logging.warning("Webull SDK not installed — using REST fallback")
            self._sdk_available = False
        except Exception as e:
            logging.warning(f"Webull SDK init failed ({e}) — using REST fallback")
            self._sdk_available = False
...
def rank_entries(
    ...
    ── NORMALIZED VARIANT ──────────────────────────────────────────────────────
    In the original engine, decisecond_score naturally weights higher timeframes:
    a 1mo hit (25,920,000 ds) outscores a 1m hit (600 ds) by 43,200×, regardless
    of how many actual OHLC hits occurred. This engine removes that bias.

    Normalization: base_ds is divided by TF_DECISECONDS[timeframe] before
    applying freshness. This converts the score to a hit-count-equivalent that
    is 1:1 across all timeframes — a 1mo signal with 5 hits and a 1m signal
    with 5 hits produce the same base score (5.0). Freshness still differentiates
    fresh vs. stale signals within each timeframe.
    ...
    for e in store.all():
        ...
        if cumulative_ds and e.key_variable is not None:
            ...
            base_ds = e.decisecond_score
        # ── NORMALIZATION: divide by timeframe baseline so all TFs score equally ──
        # Without this, a 1mo hit (25,920,000 ds) outscores a 1m hit (600 ds)
        # by 43,200× regardless of actual hit count. After normalization, each hit
        # contributes 1.0 to the base score regardless of timeframe.
        tf_ds_baseline = TF_DECISECONDS.get(e.timeframe, 600)
        base_ds = base_ds / tf_ds_baseline
...
class EngineConfig:
    universe: list[str]
    timeframes: list[str] = field(default_factory=lambda: ["1m", "5m", "15m", "30m", "1h", "2h", "4h", "1d", "1w", "1mo"])
...
class SMAOutfitEngine:
    ...
    def scan(self) -> None:
        ...
        print(f"\n  Scanning {total:,} combinations "
              f"({len(self.cfg.universe)} tickers × {len(all_tfs)} tfs × {len(self.cfg.outfits)} outfits)...", flush=True)

        # ── Parallel scan across tickers (multiprocessing — bypasses the GIL) ──
        n_workers = int(os.environ.get("NORM_SCAN_WORKERS", os.environ.get("ENGINE_SCAN_WORKERS", "6")))
        ...
        with multiprocessing.Pool(processes=n_workers, initializer=_scan_worker_init) as pool:
            for i, result in enumerate(pool.imap_unordered(_scan_worker_fn, worker_args), 1):
                ...
                print(f"  scan {i}/{n_chunks} workers done ({pct:.0f}%) — {elapsed:.0f}s elapsed, ~{eta:.0f}s remaining", flush=True)
...
def render_dashboard(signal: Optional[dict], systems: list[SystemState], top_n: list[dict]) -> str:
    lines: list[str] = []
    lines.append("═" * 71)
    lines.append("  [ NORMALIZED ENGINE — timeframe-equal scoring ]")
    if signal:
...
def save_xlsx(top_n: list[dict], path: str) -> None:
    """
    Save top-N results to a clean Excel spreadsheet.
    Columns: Rank, Ticker, Timeframe, Outfit ID, Outfit Name, Outfit Periods,
             Hit Count, Convergence, Entry Price, Norm Score, Last Hit
    """
    try:
        import openpyxl
        ...
        print("  [XLSX] openpyxl not installed — skipping spreadsheet output")
        return

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Normalized Signals"

    headers = [
        "Rank", "Ticker", "Timeframe", "Outfit ID", "Outfit Name",
        "Outfit Periods", "Hit Count", "Convergence", "Entry Price",
        "Norm Score", "Last Hit"
    ]
    ...
    for row_idx, r in enumerate(top_n, 2):
        fill = fill_even if row_idx % 2 == 0 else fill_odd
        periods_str = "/".join(str(p) for p in r.get("outfit_periods", []))
        values = [
            r.get("rank"),
            r.get("ticker"),
            r.get("timeframe"),
    ...
    # Column widths
    col_widths = [6, 10, 12, 10, 28, 35, 10, 14, 14, 12, 26]
...
NORMALIZED_TICKERS_PATH = "/app/normalized_tickers.txt"


def load_normalized_tickers(path: str = NORMALIZED_TICKERS_PATH) -> list[str]:
    """
    Load a custom ticker list for the normalized engine.

    File format (same as custom_tickers.txt / muted_tickers.txt):
      - One ticker per line
    ...
    Example /app/normalized_tickers.txt:
      # My normalized engine watchlist
      AAPL
      NVDA
    ...
    Returns empty list if file doesn't exist (falls back to --universe).
    """
    tickers = []
    try:
        with open(path) as f:
        ...
        if tickers:
            print(f"  [TICKERS] ✅ Loaded {len(tickers)} tickers from {path}")
    except FileNotFoundError:
        pass
    return tickers
...
def _check_influx() -> tuple:
    ...
    try:
        ...
        logging.warning("  [INFLUX] persistence.py not found — running in-cycle only")
        return None, {}

    persist = InfluxPersistence()

    if not persist.enabled or not persist._connected:
        logging.warning("  [INFLUX] ❌ NOT CONNECTED — cross-cycle deciseconds unavailable, running in-cycle only")
        print("  [INFLUX] ❌ NOT CONNECTED — running on in-cycle scores only")
        return persist, {}
...
def main():
    parser = argparse.ArgumentParser(description="SMA Outfit Detection Engine — Normalized (timeframe-equal scoring)")
    ...
    parser.add_argument("--universe", choices=["tier1", "tier2", "all"], default="tier1",
                        help="Base universe tier. Ignored if --tickers or normalized_tickers.txt is used.")
    parser.add_argument("--tickers", nargs="+", metavar="TICKER",
                        help="Explicit ticker list (e.g. --tickers AAPL NVDA TSLA). "
                             "Overrides --universe and normalized_tickers.txt.")
    parser.add_argument("--timeframes", nargs="+", default=["1m", "5m", "15m", "30m", "1h", "2h", "4h", "1d", "1w", "1mo"])
    ...
    parser.add_argument("--xlsx", action="store_true", help="Save results to Excel spreadsheet in output/normalized_engine/")
    ...
    # ── InfluxDB health check + cumulative deciseconds ────────────────────────
    print("\n" + "═" * 71)
    print("  NORMALIZED ENGINE — startup checks")
    print("═" * 71)
    ...
    if args.source == "webull":
        ...
        if not (app_key and app_secret):
            print("  [WEBULL] ❌ WEBULL_APP_KEY and WEBULL_APP_SECRET env vars required",
                  file=sys.stderr)
            sys.exit(1)
        client = WebullClient(app_key, app_secret, region=os.environ.get("WEBULL_REGION", "us"))
        mode = "SDK" if client._sdk_available else "REST fallback"
        print(f"  [WEBULL] ✅ Client initialised ({mode})")
    else:
        client = MockClient()
        print("  [WEBULL] ⚠️  Mock client (no real data)")

    # ── Resolve universe — priority: --tickers > normalized_tickers.txt > --universe ──
    if args.tickers:
        # Inline list from CLI
        universe = [t.upper() for t in args.tickers]
        universe_label = f"custom ({len(universe)} tickers from --tickers)"
    else:
        file_tickers = load_normalized_tickers(NORMALIZED_TICKERS_PATH)
        if file_tickers:
            universe = file_tickers
            universe_label = f"custom ({len(universe)} tickers from normalized_tickers.txt)"
        else:
            universe = build_universe(args.universe)
            universe_label = f"{args.universe} ({len(universe)} tickers)"

    # Deduplicate, preserve order
    seen_u: set = set()
    universe = [t for t in universe if not (t in seen_u or seen_u.add(t))]
    ...
    print(f"  [CONFIG] universe={universe_label} "
          f"tfs={cfg.timeframes} lookback={cfg.lookback}")
    print("═" * 71 + "\n")
    ...
    if args.xlsx:
        ...
        out_dir = "/cache/output/normalized_engine"
        _os.makedirs(out_dir, exist_ok=True)
        ts = _dt.now(_tz.utc).strftime("%Y-%m-%d_%H-%M-%S")
        xlsx_path = f"{out_dir}/normalized_{ts}.xlsx"
        save_xlsx(top_n, xlsx_path)
